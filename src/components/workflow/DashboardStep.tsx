...
import { getOrCreateSession, getUserPlansFromSupabase, UserSession } from '@/lib/supabase';
...
export default function DashboardStep({ onSelectPlan, onNewIdea, onOpenAuthModal }: DashboardStepProps) {
    ...
    try {
      if (clientSession.isLoggedIn) {
        ...
        if (fetched && fetched.length > 0) {
          ...
          // Fallback to localStorage plans if available
  ...
  if (session.isLoggedIn) {
    plans.forEach((p) => {
      const cat = p.input?.category || 'Tech';
  ...
  // Aggregated Key Innovations across plans
  const aggregatedInnovations = session.isLoggedIn 
    ? Array.from(new Set(plans.flatMap((p) => p.metrics?.keyInnovations || []))).slice(0, 6)
    : [];
    ...
    <div className="space-y-8 py-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-r from-forge-surface via-forge-surface-light to-forge-surface border border-quenched-steel/30 p-8 rounded-blueprint shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-3 z-10 shrink-0">
          {!session.isLoggedIn && onOpenAuthModal && (
            <>
              <Button
                variant="primary"
                size="md"
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row lg:flex-col items-stretch lg:items-end justify-center gap-4 shrink-0 border-t lg:border-t-0 lg:border-l border-quenched-steel/20 pt-4 lg:pt-0 lg:pl-8">
              <Button
                variant="primary"
                size="lg"
                onClick={() => onSelectPlan(mostRecentPlan)}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 pt-3 border-t border-quenched-steel/20">
                      ...
                      >
                        <FileDown className="w-4 h-4 text-brand-ember" />
                      </button>
                    </div>
                  </Card>
                );
