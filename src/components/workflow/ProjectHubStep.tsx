...
export default function ProjectHubStep({ blueprint, onOpenTelegramMentor }: ProjectHubStepProps) {
  ...
  const [inputToken, setInputToken] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('ideaforge_github_token') || '';
    }
    return '';
  });
  ...
  const handleCreateRepo = () => {
    ...
    const githubUrl = `https://github.com/new?name=${encodeURIComponent(sanitizedTitle)}&description=${encodeURIComponent(description)}`;

    // Open GitHub web repository creation page in new tab pre-filled with name & description
    window.open(githubUrl, '_blank');

    setCreatedRepoUrl(githubUrl);
